...
public class GameOver : MonoBehaviour 
{
	...
	void Start()
	{
		...
		scoresCol1 = GetHighScores(true);
		scoresCol2 = GetHighScores(false);
	}
	...
	void OnGUI()
	{
		...
		if(!showLeaderBoard && !testScript.submitted)
		{
		...
		else
		{
			...
			GUI.Box(new Rect(Screen.width * .1f, Screen.height * .2f , Screen.width * .4f,Screen.height * .6f), "High Scores\n" + GetHighScores(true) );
			GUI.Box(new Rect(Screen.width * .5f, Screen.height * .2f , Screen.width * .4f,Screen.height * .6f), " \n" + GetHighScores(false) );
	...
	string GetHighScores(bool firstCol)
	{
		...
		{ 
			...
			//allScores = client.DownloadString (getScoresURL);
			allScores = tmpLeaderBoard;
			string [] stuff = allScores.Split ('\n');



			//this is only for testing the feature out!
			leaderBoard = "";
